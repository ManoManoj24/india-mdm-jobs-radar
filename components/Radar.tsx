...
function Employer({name,jobs,generatedAt,applyTag}:{name:string,jobs:Job[],generatedAt:string,applyTag:(v:string)=>void}){const [open,setOpen]=useState(true);return <article className={`employer ${open?"expanded":"collapsed"}`}><button className="employerHead" onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-controls={`jobs-${name.replace(/\W+/g,"-")}`}><h2>{name}</h2><span>{jobs.length} {jobs.length===1?"role":"roles"}<b aria-hidden>{open?"−":"+"}</b></span></button><div className="joblist" id={`jobs-${name.replace(/\W+/g,"-")}`} hidden={!open}>{jobs.map(j=>{const age=ageDays(j.postedAt,generatedAt);return <div className={`job ${age!==null&&age>45?"older":""}`} key={j.id}><div className="jobMain"><div className="chips">{age!==null&&age<=3&&<span className="new">New</span>}{j.vendorTags.slice(0,4).map(t=><button key={t} onClick={()=>{applyTag(t);scrollTo({top:0,behavior:"smooth"})}} aria-label={`Filter by ${t}`}>{t}</button>)}</div><h3>{j.title}</h3><p className="meta">{j.location} · {j.workplace} · {j.experience}</p><p className="summary">{j.summary}</p></div><div className="jobSide"><div><span className="label">Posted</span><strong>{prettyDate(j.postedAt)}</strong>{age!==null&&<small>{age}d ago</small>}</div><a href={j.url} target="_blank" rel="noreferrer">View role <span aria-hidden>↗</span></a></div></div>})}</div></article>}
